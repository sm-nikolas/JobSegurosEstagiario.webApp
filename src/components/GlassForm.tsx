  ...
  return (
    ...
    >
      ...
      <div className="flex justify-center mb-6">
        {[1,2,3,4].map((step) => (
          <div key={step} className={`w-8 h-8 flex items-center justify-center rounded-full mx-1 text-white font-bold ${currentStep === step ? 'bg-orange-600' : 'bg-gray-300'}`}>{step}</div>
        ))}
      </div>
      ...
      <div className="flex justify-between mt-8">
        ...
        {currentStep < 4 && (
          <button type="button" onClick={nextStep} className="ml-auto bg-orange-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-orange-700 transition-colors">Próximo</button>
        )}
        {currentStep === 4 && (
          <button type="submit" className="ml-auto bg-orange-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-orange-700 transition-colors">Finalizar Contratação</button>
        )}
      </div>
    </motion.form>
  );
